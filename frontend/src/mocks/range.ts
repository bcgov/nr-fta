...
export const MOCK_RANGE_AGREEMENTS: RangeAgreementSummary[] = [
  {
    agreementId: 'RAN076543',
    agreementType: 'Grazing Licence',
    status: 'Active',
    holderName: 'Willowfield Ranch Ltd.',
    holderClient: '00120890',
  ...
  {
    agreementId: 'RAN081200',
    agreementType: 'Grazing Permit',
    status: 'Active',
    holderName: 'Grasslands Cattle Co.',
    holderClient: '00121455',
    orgUnit: 'DKA — Kamloops',
    authorizedAums: 860,
    issueDate: '2019-05-01',
    expiryDate: '2029-04-30',
  },
  {
    agreementId: 'RAN078120',
    agreementType: 'Hay Cutting Licence',
    status: 'Pending',
    holderName: 'R. Fielding',
    holderClient: '00133011',
    orgUnit: 'DND — Nadina',
    authorizedAums: 210,
  ...
  {
    agreementId: 'RAN060044',
    agreementType: 'Grazing Licence',
    status: 'Expired',
    holderName: 'Uplands Grazing Assoc.',
    holderClient: '00099210',
    orgUnit: 'DCS — Cascades',
    authorizedAums: 1540,
    issueDate: '1998-04-01',
    expiryDate: '2018-03-31',
  },
];

const DETAIL_EXTRAS: Pick<
  RangeAgreementDetail,
  'managementUnit' | 'rotations' | 'landBase' | 'usageHistory'
> = {
  ...
  landBase: [
    {
      parcelId: 'LB-01',
      description: 'Big Creek allotment — north',
      areaHa: 4200,
      tenureType: 'Crown Range',
    },
    {
      parcelId: 'LB-02',
      description: 'Big Creek allotment — south',
      areaHa: 3150,
      tenureType: 'Crown Range',
    },
  ],
...
export function searchRangeAgreements(c: RangeSearchCriteria): RangeAgreementSummary[] {
  return MOCK_RANGE_AGREEMENTS.filter((a) => {
    if (c.agreementId && !a.agreementId.toLowerCase().includes(c.agreementId.toLowerCase()))
      return false;
    if (c.holderName && !a.holderName.toLowerCase().includes(c.holderName.toLowerCase()))
      return false;
...
export function findRangeAgreement(agreementId: string): RangeAgreementDetail | undefined {
  const summary = MOCK_RANGE_AGREEMENTS.find(
    (a) => a.agreementId.toLowerCase() === agreementId.toLowerCase(),
  );
...
export const RANGE_STATUS_TAG: Record<
  RangeAgreementSummary['status'],
  'green' | 'blue' | 'gray' | 'red'
> = {
  Active: 'green',
  Pending: 'blue',
