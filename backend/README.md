...
Grouped by area; see the `controller/` package for full request/response shapes. All `/api/fta/*` routes are bearer-token-protected and require either `FTA_ADMIN` or `FTA_VIEWER` (writes are admin-only).
...
| Project search | `/api/fta/projects/search`, `.../options`, `.../file-suffixes` | List + filter-option lookups. |
| Project properties | `/api/fta/projects/{projectId}/properties` and `.../{propertyId}/contacts` | Property list + per-property contact CRUD. |
| Project contacts | `/api/fta/projects/{projectId}/contacts` (+`/options`, `/search`) | Per-project contact association CRUD. |
| Acquisition requests | `/api/fta/projects/{projectId}/acquisition-request` (+`/options`) | GET/POST/PUT — per-project AR record. |
| Admin: contacts | `/api/fta/admin/contacts` | CRUD; `FTA_ADMIN` only. |
| Admin: co-users | `/api/fta/admin/co-users` | CRUD; `FTA_ADMIN` only. |
| Admin: org units | `/api/fta/admin/org-units`, `.../{number}` | Read-only reference data. |
| Users | `/api/fta/users/search` | IDIR user search via FAM API. |
...
│   ├── FtaApiConstants.java   # Shared constants (role names, etc.)
